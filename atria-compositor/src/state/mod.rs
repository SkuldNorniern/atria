...
use crate::model::{
    ...
    Damage, Event, EventKind, FocusEvent, ObjectKind, Point, Rect, SeatCapabilities, SeatId,
    SeatSnapshot, SessionSnapshot, Size, SurfaceKey, SurfaceRole, SurfaceSnapshot, TextBuffer,
...
use crate::shell::{ShellError, ToplevelHandle};
...
const fn shell_refusal(error: ShellError, object: ObjectId) -> StateError {
    match error {
        ...
        ShellError::UnknownHandle { .. } | ShellError::UnknownSeat { .. } => {
            StateError::InvalidState { object_id: object }
        }
    }
}
